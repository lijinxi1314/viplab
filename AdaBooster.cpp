...
const string tdFile("ds_adaboost/out.td");
...
WeakClassifier* AdaBooster::get_best_classifier(){
	...
	////////////////////////////////////////////
	cout << endl;
	cout << "accuracy: " << 1 - lowest_err << endl;
	cout << "position : " << dimension << endl;//(선택된 feature에서 어디 부분인지)
	cout << "feature_index : " << index << endl;//(선택된 feature 의 고유 number)
	cout << "threshold: " << threshold << endl;//sorted[dimension][index]->at(dimension) : 선택된 feature에서 해당 위치에 해당하는 값(featue값)
	//////////////////////////////////
...
bool AdaBooster::update_feature_weight(TrainingData &td, WeakClassifier &wc){
	...
	for (unsigned int i=0; i<num_features; i++){
		...
		// note M_E := 2.71828
		float weight = pow(2.71828, (double) -1 * wc.weight() * is_correct);
